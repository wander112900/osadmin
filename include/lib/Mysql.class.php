...
class Mysql{
	...
	private function initPDO($master = TRUE) {
		...
		if ($master) {
			/*
			$dsn = sprintf ( $dsn, $this->config ['master'] ['host'], $this->config ['dbname'], '3306' );
			$username = $this->config ['master'] ['username'];
			$password = $this->config ['master'] ['password'];
			*/
			$dsn = sprintf ( $dsn, $this->config ['server'], $this->config ['db_name'], $this->config ['port'] );
			$username = $this->config['username'];
			$password = $this->config['password'];
			
		} else {
			$dsn = sprintf ( $dsn, $this->config ['slave'] ['host'], $this->config ['dbname'], '3306' );
